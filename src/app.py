...
class PipersFriendApp:
    ...
    def action_export_bww(self): messagebox.showinfo("BWW", "BWW pipeline structural track exported.")
    ...
    def setup_ribbon_tab_buttons(self):
        tabs = ["File", "Notes", "Gracenotes", "Bar", "Playback", "Settings"]
        self.ribbon_buttons = {}
        for tab in tabs:
    ...
    def activate_ribbon_tab(self, tab_name):
        ...
        if tab_name == "File":
        ...
        elif tab_name == "Gracenotes":
            graces = [("Gracenote", None), ("D Throw", "d_throw"), ("Birl", "birl"), ("High G Birl", "high_g_birl"), ("Doubling", "doubling")]
            ...
            if self.selected_bar is None:
                ...
                tk.Label(self.ribbon_subpanel, text=f"Staff {staff}, Bar {b + 1}",
                         fg="#f59e0b", bg="#202023", font=("Arial", 9, "bold")).pack(side=tk.LEFT, padx=(8, 12))
                ...
                tk.Label(self.ribbon_subpanel, text="End:", fg="white", bg="#202023").pack(side=tk.LEFT, padx=(4, 2))
    ...
    def set_active_gracenote_embellishment(self, emb):
        ...
        self.activate_ribbon_tab("Gracenotes")
    ...
    def identify_closest_staff_and_pitch(self, y_pos):
        dist_s1 = abs(y_pos - STAF_1_Y)
        staf_2_y = STAF_1_Y + self.score.gap_between_staves
        dist_s2 = abs(y_pos - staf_2_y)
        target_staff, base_y = (1, STAF_1_Y) if dist_s1 < dist_s2 else (2, staf_2_y)
        
        if y_pos < (base_y - 22) or y_pos > (base_y + STAFF_H + 22): return None, None, None
    ...
    def handle_canvas_click(self, event):
        ...
        staf2_y = STAF_1_Y + self.score.gap_between_staves
        band_staff = None
        if STAF_1_Y - 12 <= y <= STAF_1_Y + STAFF_H + 12:
            band_staff = 1
        elif staf2_y - 12 <= y <= staf2_y + STAFF_H + 12:
            band_staff = 2
        ...
        for node in self.score.nodes:
            if node.get("render_x") and node.get("render_y"):
                if abs(node["render_x"] - x) <= hit_margin and abs(node["render_y"] - y) <= hit_margin:
                    ...
                    self.selected_bar = (node.get("staff", 1), node.get("bar_index", 0))
                    if self.active_tab != "Notes":
                        self.activate_ribbon_tab("Notes")
                    self.redraw_canvas_score()
                    return

        # Clicking empty stave selects that bar (for the Bar menu).
        if band_staff is not None:
            self.selected_bar = (band_staff, self.get_bar_index_from_x(x, band_staff))
            if self.active_tab == "Bar":
                self.activate_ribbon_tab("Bar")   # refresh the Bar menu buttons

        # Notes are only placed while the Notes or Gracenotes tab is open.
        if self.active_tab not in ("Notes", "Gracenotes"):
            self.redraw_canvas_score()
            return
        ...
        if is_grace and self.selected_embellishment:
            ...
            # Embellishment placement rules.
            if emb in ("birl", "high_g_birl") and target_pitch != "Low A":
                self.status_label.config(text="⚠️ Birls can only be played on Low A.")
                return
            if emb == "d_throw" and target_pitch != "D":
                self.status_label.config(text="⚠️ A Throw on D can only be attached to D.")
                return
            if emb == "d_throw":
                pitches = ["Low G", "E", "D"]
            elif emb == "birl":
                pitches = ["Low G", "Low A", "Low G"]
            elif emb == "high_g_birl":
                pitches = ["High G", "Low A", "Low G", "Low A", "Low G"]
            elif emb == "doubling":
                # The doubling is tied to whichever note it's attached to.
                pitches = DOUBLING_MAP.get(target_pitch, ["High G", target_pitch])
            else:
                pitches = ["Low G"]
    ...
    def handle_canvas_drag(self, event):
        if self.dragging_bar is not None:
            ...
            z = self.zoom or 1.0
            base_y = STAF_1_Y if staff == 1 else STAF_1_Y + self.score.gap_between_staves
            x = lines[idx] * z
            self.canvas.delete("dragbar")
            self.canvas.create_line(x, base_y * z, x, (base_y + STAFF_H) * z,
                                    fill="#2563eb", width=2, tags="dragbar")
    ...
    def _build_page_buffer(self):
        ...
        staf2_y = STAF_1_Y + self.score.gap_between_staves
        self.draw_staff_background_grid(draw, STAF_1_Y, scale, 1)
        self.draw_staff_background_grid(draw, staf2_y, scale, 2)
        self._draw_bar_styles(hr_image, STAF_1_Y, scale, 1)
        self._draw_bar_styles(hr_image, staf2_y, scale, 2)

        if self.clef_image:
            # Scale the clef keeping its aspect, then place it so the spiral
            # belly sits on the G line (Low G, 4th line = base_y + 3*LINE_GAP).
            clef_h = 6.0 * LINE_GAP
            iw, ih = self.clef_image.size
            clef_w = clef_h * iw / ih
            c_w, c_h = max(1, int(clef_w * scale)), max(1, int(clef_h * scale))
            clef_r = self.clef_image.resize((c_w, c_h), Image.Resampling.LANCZOS)
            clef_x = int((CANVAS_LEFT + 8) * scale)
            for base_y in (STAF_1_Y, staf2_y):
                g_line = base_y + 3 * LINE_GAP
                top = g_line - CLEF_CURL_FRAC * clef_h
                hr_image.paste(clef_r, (clef_x, int(top * scale)), clef_r)

        ts_raw = self.score.time_signature.strip().lower()
        if ts_raw in ["c", "common"]:
            ts_filename = "common.png"
        elif ts_raw in ["cut", "c|"]:
            ts_filename = "cut.png"
        else:
            ts_filename = ts_raw.replace("/", "_") + ".png"

        ts_path = self._asset_path("time_sigs", ts_filename)
        ...
        if os.path.exists(ts_path):
            try:
                ...
                # The time signature spans the whole stave, centred on the B line
                # (the middle line, = base_y + 2*LINE_GAP).
                ts_h = STAFF_H
                iw, ih = ts_img.size
                ts_w = ts_h * iw / ih
                ts_r = ts_img.resize((max(1, int(ts_w * scale)), max(1, int(ts_h * scale))),
                                     Image.Resampling.LANCZOS)
                ts_x = int((CANVAS_LEFT + 48) * scale)
                for b_y in (STAF_1_Y, staf2_y):
                    b_line = b_y + 2 * LINE_GAP
                    top = b_line - ts_h / 2
                    hr_image.paste(ts_r, (ts_x, int(top * scale)), ts_r)
            except Exception:
                pass
        ...
        for node in self.score.nodes:
            ...
            base_y = STAF_1_Y if node.get("staff", 1) == 1 else staf2_y
            node["render_x"] = node.get("raw_x", 140)
            node["render_y"] = base_y + pitch_cfg["y_offset"]
        ...
        # per the bagpipe beaming rules: beams never cross a beat boundary, the
        # beam count matches the note value, and partial beams clarify mixed
        # values. Individual flags are not used.
        is_compound = ts_raw in ("6/8", "9/8", "12/8")
        beat_size = 0.75 if is_compound else 0.5  # dotted crotchet vs crotchet
        ...
        for tgt, members in grace_by_target.items():
            ...
            g_base = STAF_1_Y if g_staff == 1 else STAF_1_Y + self.score.gap_between_staves
    ...
    def render_canvas_interactive_overlay(self):
        ...
        # Hover preview: a translucent bluish image of the note/grace that would
        # be placed. Only shown while a placeable tool is active (Notes/Gracenotes
        # tab); otherwise it disappears.
        if (self.hover_x is not None and self.hover_y is not None
                and self.active_tab in ("Notes", "Gracenotes")):
            ...
            if staff_num is not None:
                ...
                dur_type = self.selected_duration.lower()
                prev = self._hover_preview(dur_type, z)
                if prev is not None:
                    ph, ax, ay = prev
    ...
    def _hover_preview(self, dur_type, zoom):
        """A translucent bluish PhotoImage of the glyph for ``dur_type`` at the
        current zoom. Returns ``(PhotoImage, anchor_x, anchor_y)`` or ``None``."""
        key = (dur_type, round(zoom, 3))
        ...
        blue = Image.new("RGBA", small.size, (59, 130, 246, 0))  # bluish
        blue.putalpha(alpha)
        ph = ImageTk.PhotoImage(blue)
        entry = (ph, ax * f, ay * f)
    ...
    def _run_audio_synthesis_loop(self, mode):
        try:
            ...
            return
        try:
            # Play in reading order (staff, bar, x), gracenotes included.
            ...
            if mode == "note" and self.selected_node_id:
                ...
                nodes = nodes[idx:]
            elif mode == "bar" and self.selected_node_id:
                sel = next((n for n in nodes if n["id"] == self.selected_node_id), None)
                if sel:
                    nodes = [n for n in nodes if n.get("staff") == sel.get("staff")
                             and n.get("bar_index") == sel.get("bar_index")]
